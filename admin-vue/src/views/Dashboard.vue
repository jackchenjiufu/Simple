...
onMounted(async()=>{
  try{const rs=await api.get('/admin_stats.php');if(rs.data.code===200&&rs.data.data){const d=rs.data.data;statItems.value=[{value:d.total_users||'--',label:'用户数'},{value:d.total_content||'--',label:'内容数'},{value:d.total_follows||'--',label:'关注数'},{value:d.total_messages||'--',label:'消息数'}]}}catch(e){}
