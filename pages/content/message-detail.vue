		...
		async loadMessageDetail() {
			try {
				...
				if (this.message) {
					...
					} else if (this.message.sender_id && this.message.sender_id !== (uni.getStorageSync('userInfo') || {}).id) {
						// 如果没有other_user_id，但有sender_id且不是当前用户，则使用sender_id作为对方用户ID
						this.userId = this.message.sender_id;
					} else if (this.message.receiver_id && this.message.receiver_id !== (uni.getStorageSync('userInfo') || {}).id) {
				...
				if (response && (response.code === 200 || response.success)) {
					...
					// 设置对方用户ID，确保聊天历史API能获取正确的聊天记录
					const currentUserId = parseInt((uni.getStorageSync('userInfo') || {}).id || 0);
